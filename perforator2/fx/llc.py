...
def ratio(sh, iso, param):
  ...
  # llcmap = {}
  # for bmark, sh, iso in sorted(dictzip(shared, isolated)):
  #   ratio = iso['LLC-stores'] / iso ['cycles']
  #   llcmap[bmark] = ratio
  # print(sorted(llcmap.items(), key=(lambda k: k[1])))
    # perf_r = ratio(sh, iso, 'instructions')
    # llc_r = ratio(sh, iso, 'LLC-load-misses')

    # llc_r = sh['LLC-load-misses']/iso['LLC-load-misses']
    # print("{bmark:<14}: {perf_r:.3f} {llc_r:.3f}".format(perf_r=perf_r, llc_r=llc_r, bmark=bmark))

    # for d in [iso, sh]:
    #   insn = d['instructions']
    #   cycles = d['cycles']
    #   misses = d['LLC-load-misses']
    #   print("miss cost", (cycles - insn) / misses)
...
results = {
  'fx_qnc': {
    ...
    # ~ 2H
    'joint': {('sdagp', 'wordpress'): [0.3430965814443583, 0.6365587555884425], ('ffmpeg', 'wordpress'): [1.0371735256331283, 0.6172619689305328], ('matrix', 'wordpress'): [1.1623467669678447, 0.5639848862800257], ('pgbench', 'pgbench'): [0.28722618220913865, 0.28709351888619156], ('blosc', 'blosc'): [0.7025859812564249, 0.7027715000892913], ('ffmpeg', 'sdag'): [1.080037286291761, 0.7977716815055407], ('sdagp', 'static'): [0.33730711593981755, 0.6854684429849044], ('ffmpeg', 'pgbench'): [0.9891276415528788, 0.3226154637323915], ('sdag', 'wordpress'): [0.7928237256451539, 0.6475012352180136], ('blosc', 'sdagp'): [0.7824750410519876, 0.3241118332968762], ('pgbench', 'sdag'): [0.33480011720968217, 0.7579838913352915], ('sdag', 'static'): [0.7234628348873473, 0.7095840479048232], ('matrix', 'static'): [0.9353918747373895, 0.6196772582470589], ('blosc', 'sdag'): [0.7842055974955036, 0.7700028477168415], ('blosc', 'ffmpeg'): [0.7693863497446234, 1.043561520431615], ('ffmpeg', 'static'): [0.9710992869848395, 0.6868290179195146], ('ffmpeg', 'matrix'): [0.9754557463812821, 1.1227619197438612], ('matrix', 'sdag'): [1.1932720727886252, 0.7247413614793661], ('pgbench', 'wordpress'): [0.3142125959315135, 0.5697829822839143], ('ffmpeg', 'ffmpeg'): [1.0146541626178265, 1.0127402742621707], ('static', 'static'): [0.6037023380310536, 0.6059869746359225], ('pgbench', 'static'): [0.27572877510582033, 0.6584913054632456], ('pgbench', 'sdagp'): [0.3083721206947051, 0.3764763257495822], ('blosc', 'wordpress'): [0.7798164094890779, 0.6085187872840804], ('matrix', 'pgbench'): [1.1117117304701223, 0.27317619197466153], ('wordpress', 'wordpress'): [0.5925345116240807, 0.5883107667267231], ('blosc', 'static'): [0.6954315879927159, 0.6480019596008798], ('static', 'wordpress'): [0.6773638363494453, 0.5606111166562152], ('sdag', 'sdagp'): [0.7705793608170706, 0.3419872210713658], ('blosc', 'pgbench'): [0.7660014096432362, 0.2969626005969703], ('matrix', 'matrix'): [0.8348071133815028, 0.8353572578683788], ('matrix', 'sdagp'): [1.1388142783070707, 0.269629343036394], ('sdagp', 'sdagp'): [0.3276833203924927, 0.34010497284277147], ('blosc', 'matrix'): [0.6823663368253011, 1.0420031109332408], ('sdag', 'sdag'): [0.765633349733418, 0.77455869164682], ('ffmpeg', 'sdagp'): [1.094010419099382, 0.41054127238945776]}
  },
  'fx_normale': {
    # ./perforator.py -t "func=isolated_performance,interval=180000,warmup=15" -b
    # 27m
  ...
  'fx_far_and_dirty':
    OrderedDict([(('blosc', 'blosc'), [0.8791980020581556, 0.8835425143956932]),
